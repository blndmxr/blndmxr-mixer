...
export default function App() {
  ...
  if (!existingDbs && hasIDB != undefined) {
    if (!hasIDB) {
      return <p>Indexeddb is not working. Please allow indexeddb if you're in a private (TOR) window!</p>;
    }
  }
  if (!existingDbs) {
    return <p>Loading...</p>;
    ...
    return (
      <Router>
        <Switch>
          ...
          <Route render={(props) => <CreateWallet isMobile={mobileView} setIsWalletSet={setIsWalletSet} />} />
        </Switch>
      </Router>
    );
  }
